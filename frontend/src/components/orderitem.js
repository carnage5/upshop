...
function Singleorder(props) {
    ...
    return ( 
        ...
        <div className="w-[65%] p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
        ...
        {item ?<div className="grid grid-flow-row gap-4  items-center  " onClick={test}>{props.data.products.map((t) => (
                    <Item key={t.product} id={t.product} quan={t.quantity}></Item> 
                ))}</div>:<button onClick={test} className="text-white bg-red-700 hover:bg-red-800  focus:ring-red-300 font-medium rounded-lg p-2 text-center dark:bg-red-600 dark:hover:bg-red-700">Show items</button>}
